...
void Epoll::epoll_add(SP_Channel request, int timeout) {
  ...
  if (timeout > 0) {
    ...
    fd2http_[fd] = request->getHolder();//修改的时候没有了这一句
  }
  //创建一个epoll_event把fd和其对应起来
...
// 修改描述符状态，epoll_wait函数会将可以处理的fd的事件类型置空，但是fd本身并未清空，所以不用add，直接mod即可。
...
std::vector<SP_Channel> Epoll::poll() {
  while (true) {
    int event_count =
        epoll_wait(epollFd_, events_.data(), events_.size(), EPOLLWAIT_TIME);
